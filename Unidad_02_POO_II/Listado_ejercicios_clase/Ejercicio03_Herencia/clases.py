...
class Periodico(Publicacion):
    ...
    def __str__(self):
        return (
            f"Periodico: {self.titulo} ({self.anyo}) de {self.autor} - {self.seccion}"
        )
    ...
    def contar_publicaciones_tipo(cls, tipo):
        return len(
            [
                ...
                and tipo.lower() == "libro"
                or isinstance(p, Revista)
                and tipo.lower() == "revista"
                or isinstance(p, Periodico)
                and tipo.lower() == "periodico"
            ]
        )
